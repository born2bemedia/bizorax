...
type DeliverablePayload = { url?: string; filename?: string } | number | null;
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const resolveUploadUrl = (upload: InvoicePayload | DeliverablePayload): string | null => {
      if (upload != null && typeof upload === "object" && "url" in upload && upload.url) {
        return upload.url.startsWith("http") ? upload.url : `${SERVER_URL}${upload.url}`;
      }
      return null;
    };

    const orders = rawOrders.map((order) => {
      const invoiceDownloadUrl = resolveUploadUrl(order.invoice);
      const deliverablesDownloadUrl = resolveUploadUrl(order.deliverables);
