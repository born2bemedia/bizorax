...
if (SENDGRID_API_KEY) {
  ...
  console.warn('[createOrder] SENDGRID_API_KEY is not set — order emails will NOT be sent');
}
...
const postOrder = async (
  ...
  if (!SENDGRID_API_KEY || !FROM_EMAIL || !ADMIN_EMAIL) {
    console.warn('[createOrder] Skipping order emails — missing env vars:', {
    ...
    try {
      ...
      console.log(`[createOrder] Sending admin order email to: ${ADMIN_EMAIL}`);
      try {
        const [adminRes] = await sgMail.send(adminMsg);
        console.log(`[createOrder] Admin email sent — statusCode: ${adminRes.statusCode}`);
      } catch (err) {
        const sgErr = err as { message?: string; response?: { statusCode?: number; body?: unknown } };
        console.error('[createOrder] Admin email failed:', sgErr.message);
        if (sgErr.response) {
          console.error('[createOrder] SendGrid status:', sgErr.response.statusCode);
          console.error('[createOrder] SendGrid body:', JSON.stringify(sgErr.response.body, null, 2));
        }
      }

      console.log(`[createOrder] Sending order confirmation to: ${data.email}`);
      try {
        const [userRes] = await sgMail.send(userMsg);
        console.log(`[createOrder] User email sent — statusCode: ${userRes.statusCode}`);
      } catch (err) {
        const sgErr = err as { message?: string; response?: { statusCode?: number; body?: unknown } };
        console.error('[createOrder] User email failed:', sgErr.message);
        if (sgErr.response) {
          console.error('[createOrder] SendGrid status:', sgErr.response.statusCode);
          console.error('[createOrder] SendGrid body:', JSON.stringify(sgErr.response.body, null, 2));
        }
      ...
      console.error('[createOrder] Unexpected error in email block:', emailError);
    }
  }
